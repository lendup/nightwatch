...
Util.retryNTimes = function(i, requestFactory, success, err) {
  if (i < 0) {
    throw new Error('Negative number of retries specified');
  }
  ...
  request
    .on('success', function() {
        var args = Array.prototype.slice.call(arguments, 0);
        success.apply(null, [request].concat(args));
    })
    .on('error', function() {
      if (i == 0) {
        var args = Array.prototype.slice.call(arguments, 0);
        err.apply(null, [request].concat(args));
      } else {
        var times = i == 1 ? ' more time.' : ' more times.';

        console.log('\n' + Logger.colors.yellow('Request failed. Retrying ' + i + times));
        Util.retryNTimes(i - 1, requestFactory, success, err);
      }
    })
    .send();
};
